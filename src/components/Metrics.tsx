    ...
    >
      ...
      <div className="flex shrink-0 animate-marquee">
        {duplicatedMetrics.map((metric, i) => (
          <div
            key={`first-${i}`}
            className="flex min-w-[200px] md:min-w-[280px] shrink-0 flex-col items-center justify-center p-4 gap-1 lg:gap-2 border-r border-white/20"
          >
            <h4 className="font-sans font-bold text-lg md:text-xl">
              {metric.value}
            </h4>
            <p className="font-sans text-[12px] md:text-[14px] md:text-sm text-white/80 max-w-[160px] leading-tight capitalize">
              {metric.label}
            </p>
          </div>
        ))}
