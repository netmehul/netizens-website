      ...
      <div className="w-full max-w-[1320px] mx-auto px-4 relative z-10 flex flex-col gap-10">
        <div className="flex flex-col items-center justify-center gap-2 text-center">
          <div className="bg-white/10 border border-[#0E3572]/10 flex items-center justify-center px-[18px] py-[8px] rounded-[4px]">
            <span className="font-sans text-[12px] text-snow-white/90 tracking-wide">
              How we think about partnerships
            </span>
          </div>
          <h2 className="font-headings font-normal text-3xl md:text-[36px] leading-snug text-snow-white">
            We stay when others step away.
          </h2>
          <p className="font-sans text-sm text-snow-white/70 leading-[22px]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4">
          {cards.map((card, idx) => (
            ...
            >
              ...
              <p className="font-headings text-base md:text-lg text-snow-white leading-snug">
                {card.description}
              </p>
              <Link
                to="/how-we-work"
                className="font-sans text-[11px] font-semibold text-snow-white/70 uppercase tracking-wider mt-auto hover:text-snow-white transition-colors"
              >
                {card.label}
              </Link>
            </div>
