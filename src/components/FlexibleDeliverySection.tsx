        ...
        <section className="relative w-full bg-white py-16 md:py-14">
            <div className="w-full max-w-7xl mx-auto px-4 flex flex-col gap-10">
                <div className="flex flex-col items-center justify-center gap-4 text-center">
                    ...
                    <h2 className="font-headings font-normal text-3xl md:text-[38px] leading-tight text-carbon-black">
                        Flexible Delivery, Built Around You.
                    </h2>
                    <p className="font-sans font-medium text-sm md:text-base text-charcoal max-w-2xl">
                ...
                <div className="flex flex-col lg:flex-row gap-2 lg:gap-4 w-full">
                    ...
                    <div className="lg:hidden w-full flex flex-col gap-3">
                        {DELIVERY_MODELS.map((model) => (
                            <div
                                key={model.id}
                                className={`rounded-lg overflow-hidden border border-[#e5e7eb] transition-all duration-300 ${
                                    activeId === model.id ? 'bg-white shadow-md' : 'bg-[#FAF5F5] shadow-none'
                                }`}
                            >
                                <button
                                    onClick={() => setActiveId(activeId === model.id ? '' : model.id)}
                                    className={`w-full flex items-center justify-between px-5 py-4 text-left font-sans font-semibold text-sm transition-colors ${
                                        activeId === model.id ? 'text-carbon-black' : 'text-carbon-black hover:bg-white/50'
                                    }`}
                                >
                                    {model.title}
                                    <ChevronDown
                                        className={`w-5 h-5 text-charcoal shrink-0 transition-transform duration-200 ${
                                            activeId === model.id ? 'rotate-180' : ''
                                        }`}
                                    />
                                </button>
                                {activeId === model.id && (
                                                ...
                                                <ul className="flex flex-col gap-2">
                                                    {model.whatYouGet.map((item, i) => (
                                                        <li key={i} className="flex items-start gap-3">
                                                            ...
                                                            <span className="font-sans text-sm text-charcoal leading-[22px]">{item}</span>
                                                        </li>
                                                    ))}
                                                </ul>
                                            </div>
                                                    ...
                                                    {model.howItWorks.map((item, i) => (
                                                        <li key={i} className="flex gap-3">
                                                            ...
                                                            <div>
                                                                ...
                                                                <p className="font-sans text-sm text-charcoal leading-[22px] mt-0.5">{item.description}</p>
                                                            </div>
                                                        </li>
                                                    ))}
                                                </ol>
                    ...
                    <div className="hidden lg:flex flex-col gap-3 w-[280px] shrink-0">
                        {DELIVERY_MODELS.map((model) => (
                            <button
                                ...
                                className={`w-full text-left px-5 py-4 rounded-lg border font-sans font-semibold text-sm md:text-base transition-all duration-300 ease-out ${
                                    activeId === model.id
                                        ? 'bg-white border-regal-navy/25 text-carbon-black shadow-md shadow-regal-navy/5 ring-1 ring-regal-navy/10'
                                        : 'bg-[#FAF5F5] border-[#e5e7eb] text-charcoal shadow-none hover:bg-[#f5f0f0] hover:border-regal-navy/10 active:scale-[0.995]'
                                }`}
                            >
                                {model.title}
                                {activeId === model.id && (
                                    <div className="mt-4 pt-4 border-t border-regal-navy/10 animate-fade-in-down">
                                        ...
                                        <ul className="flex flex-col gap-3">
                                            {model.whatYouGet.map((item, i) => (
                                                <li key={i} className="flex items-start gap-3 opacity-0 animate-fade-in" style={{ animationDelay: `${0.15 + i * 0.06}s`, animationFillMode: 'forwards' }}>
                                                    <img src={iconTick} alt="" className="shrink-0 mt-0.5 w-5 h-5" />
                                                    <span className="font-sans text-sm text-charcoal leading-[22px]">
                                                        {item}
                                                    </span>
                                                </li>
                                            ))}
                                    ...
                                    {active.howItWorks.map((item, i) => (
                                        <li key={i} data-delivery-step className="flex gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="font-sans text-sm text-charcoal leading-[22px] mt-1">
                                                    {item.description}
                                                </p>
                                            </div>
                                        </li>
