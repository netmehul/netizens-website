    ...
    return (
        <div className={`flex flex-wrap items-center gap-[16px] w-full ${className}`}>
            {items.map((item, index) => {
                ...
                const textColorClass = isFirst ? 'text-charcoal' : 'text-charcoal/40';

                return (
                    <React.Fragment key={index}>
                        <div className="flex py-[16px] gap-[4px] items-center shrink-0">
                            {item.href && !isLast ? (
                                <a
                                    href={item.href}
                                    className={`font-sans font-medium text-[12px] uppercase transition-colors hover:text-charcoal ${textColorClass}`}
                                >
                                    {item.label}
                                </a>
                            ) : (
